...
```
+---------------------------------------------------------------+
| 侧边栏 (16px/250px) |        主聊天区 (自适应)         | 知识面板 (300px固定) |
| 可折叠图标轨          | 中间弹性布局                    | 右侧固定             |
+---------------------------------------------------------------+
...
**具体尺寸规范：**
- **侧边栏**：宽度支持两档切换：16px (图标轨模式) 或 250px (展开模式)。左侧固定定位。
- **主聊天区**：自适应宽度，最小宽度400px，最大宽度无限制。
- **知识面板**：宽度300px，右侧固定定位，默认折叠状态。
- **输入框**：全宽度，最小高度72px，最大高度200px。

#### 形状和视觉层次

**容器形状：**
- **圆角系统**：外层容器无圆角，内部卡片（消息气泡、输入框、面板内容）使用 12px-20px 圆角。
- **阴影层次**：
  - 侧边栏：无阴影，仅 1px 边框。
  - 消息气泡：`--shadow-sm`。
  - 代码块/推理链：`--shadow-xl` (提升层级感)。
  - 输入框：`--shadow-lg`。
- **分割线**：1px 实线 `var(--border)`。
