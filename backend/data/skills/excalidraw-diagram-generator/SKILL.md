...
- L1: Basic diagram generation (no icon library required)
- L2: Professional icon diagram generation (icon library required)
- L3: Auto enhancement (icon + arrows + labels)

### Level Triggers

- L1 is active when only natural language input is available and no icon library is requested or available.
- L2 is active when the requested icon library exists and passes basic integrity checks (`reference.md` and non-empty `icons/`).
- L3 is active when L2 preconditions are met and post-processing actions (`add-arrow.py`, optional labels) complete successfully.

### Downgrade and Fallback Paths

- L3 -> L2: If arrow/label enhancement fails, keep generated icon diagram, record warning, and return the partially enhanced file.
- L2 -> L1: If icon library is missing, malformed, or icon lookup fails, continue with base shapes and text labels.
- L3 -> L1: If both icon integration and enhancement steps fail, return a valid base diagram and surface recovery instructions.
- Any level -> Read-only guidance: If file write conflicts or invalid output path persists after one retry, stop mutation and return actionable commands for manual recovery.

### Failure Prompt Templates

- Missing library:
  `Icon library '{library_name}' is not available. I can continue in L1 with basic shapes now, or you can set up the library by running: python skills/excalidraw-diagram-generator/scripts/split-excalidraw-library.py skills/excalidraw-diagram-generator/libraries/{library_name}/`
- Parameter error:
  `I cannot run '{action_name}' because parameter '{parameter}' is invalid: {reason}. Please provide a corrected value, or I can continue with a simplified L1 diagram.`
- File conflict:
  `Target file '{output_file}' already exists. I can write to '{output_file}.edit' to avoid overwrite, or use a new file name you provide.`
- `.edit` suffix conflict:
  `The fallback file '{output_file}.edit' also exists. Please provide a unique output name, or allow overwrite explicitly so I can proceed safely.`
