...
def rollback_edit_path_on_error(work_path: Path | None, final_path: Path | None) -> None:
    """Best-effort rollback to restore original file name when edit flow fails."""
    if work_path is None or final_path is None:
        return
    if final_path.exists() or not work_path.exists():
        return
    work_path.rename(final_path)
...
def main():
    ...
    try:
        ...
        rollback_edit_path_on_error(work_path, final_path)
        print(f"Error: {e}")
        sys.exit(1)
