    ...
    McpTemplate(
        ...
        fields=[
            ...
            _field(
                ...
                type="json",
                required=True,
                default_value='["-y", "your-company-jira-mcp-package"]',
                help_text="Replace the placeholder package or executable with the real Jira MCP implementation your company uses.",
            ),
            _field(
                "baseUrl",
                "Jira Base URL",
            ...
            _field(
                ...
                help_text="Leave the username blank if your MCP server authenticates with only a personal token.",
            ),
            _field(
                "secretEnvVar",
                "Host Personal Token Env Var",
                required=True,
                default_value="JIRA_TOKEN",
                placeholder="JIRA_TOKEN",
                help_text="The rendered config stores ${ENV_NAME} so the personal token stays outside Yue.",
            ),
            _field(
                "tokenEnvKey",
                "Personal Token Env Key For MCP Server",
                required=True,
                default_value="JIRA_TOKEN",
                help_text="Use the exact personal-token env var name expected by your Jira MCP server.",
            ),
            _field(
                "extraEnvJson",
