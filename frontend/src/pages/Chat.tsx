                     ...
                     <div class="relative space-y-5">
                        {(() => {
                          ...
                          return (
                            <>
                              <Show when={thought}>
                                <div class="bg-background/80 border border-border/60 rounded-2xl overflow-hidden transition-all duration-500 shadow-inner group/thought">
                                  <button 
                                    onClick={() => toggleThought(index())}
                                    class="w-full flex items-center justify-between px-5 py-3 text-[10px] font-black text-text-secondary hover:bg-primary/5 transition-all uppercase tracking-[0.15em]"
                                  >
                                    <div class="flex items-center gap-3">
                                      <div class="relative">
                                        <div class={`w-2 h-2 rounded-full ${isTyping() && !content ? 'bg-primary animate-ping' : 'bg-primary/40'}`}></div>
                                        <div class={`absolute inset-0 w-2 h-2 rounded-full ${isTyping() && !content ? 'bg-primary' : 'bg-primary/60'}`}></div>
                                      </div>
                                      <span class={isTyping() && !content ? 'text-primary' : ''}>
                                        {isTyping() && !content ? 'Synthesizing Intelligence...' : 'Reasoning Protocol'}
                                      </span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                      <span class="opacity-0 group-hover/thought:opacity-100 transition-opacity text-[9px] font-black">
                                        {expandedThoughts()[index()] ? 'SECURE_COLLAPSE' : 'SECURE_EXPAND'}
                                      </span>
                                      <svg xmlns="http://www.w3.org/2000/svg" class={`h-4 w-4 transition-transform duration-500 ease-out ${expandedThoughts()[index()] ? 'rotate-180' : ''}`} viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                                      </svg>
                                    </div>
                                  </button>
                                  <Show when={expandedThoughts()[index()]}>
                                    <div class="px-5 pb-5 text-[13px] text-text-secondary/80 font-medium leading-relaxed border-t border-border/40 pt-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                      <div class="prose prose-xs prose-p:my-1 prose-pre:my-2 prose-pre:bg-black/5 dark:prose-pre:bg-white/5 prose-pre:p-3 prose-pre:rounded-xl opacity-80 italic">
                                        {thought}
                                      </div>
                                    </div>
                                  </Show>
