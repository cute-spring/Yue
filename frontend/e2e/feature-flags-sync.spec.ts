...
test('trace ui feature flag saved in settings becomes visible in chat on return', async ({ page }) => {
  ...
  await expect(page.getByTestId('settings-feature-flag-chat-trace-ui')).not.toBeChecked();

  await page.getByTestId('settings-feature-flag-chat-trace-ui').check();
  await page.getByTestId('settings-save-feature-flags').click();
