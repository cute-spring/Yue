...
- [ ] **Step 1: 写失败单测（SkillGroupConfig 模型 + 基础 CRUD）**
...
- [ ] **Step 1: 写失败 API 测试（list/create/update/delete）**
- [ ] **Step 2: 运行测试，确认失败**
...
- [ ] **Step 1: 写失败测试（group + extra + legacy visible_skills 合并去重）**
- [ ] **Step 2: 运行测试确认失败**
...
- [ ] **Step 1: 写失败测试（manual/auto 模式下使用 resolved skills）**
- [ ] **Step 2: 运行测试确认失败**
...
- [ ] **Step 1: 新增 E2E 失败用例（切换类型、选择 group、保存回显）**
- [ ] **Step 2: 运行 E2E 确认失败**

Run: `cd frontend && npm run test:e2e -- skills-runtime-ui.spec.ts`  
Expected: FAIL

- [ ] **Step 3: 实现表单交互与提交 payload**
- [ ] **Step 4: 运行 E2E 确认通过**
...
- [ ] **Step 1: 写失败 E2E（通用 agent 下 skill selector 正常可选）**
- [ ] **Step 2: 运行 E2E 确认失败**

Run: `cd frontend && npm run test:e2e -- skills-runtime-ui.spec.ts`  
Expected: FAIL

- [ ] **Step 3: 实现优先使用 `resolved_visible_skills` 的渲染逻辑**
- [ ] **Step 4: 再跑 E2E 确认通过**
...
- [ ] **兼容性风险：** 老 agent 无 `agent_kind` 字段导致行为偏差  
  缓解：后端默认值 + 迁移脚本 dry-run 报告
- [ ] **配置膨胀风险：** group 与 extra_visible_skills 叠加导致理解困难  
  缓解：UI 提供“最终可见技能数”和来源标签
- [ ] **路由误选风险：** 可见技能增多后 auto 模式误判提升  
...
- [ ] 支持两类 agent：`traditional` / `universal`，并可在 UI 配置
- [ ] 支持 skill group CRUD，并可挂载到多个 agent
- [ ] runtime 使用解析后的可见技能集合，旧 `visible_skills` 仍可兼容
- [ ] manual / auto 路由与工具交集策略保持稳定
- [ ] 新增测试通过，`./check.sh` 可执行并记录结果
- [ ] 提供可回滚迁移路径与运行观测指标
