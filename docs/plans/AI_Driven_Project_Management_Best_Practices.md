...
## 3. 历史遗留计划的收敛与排序 (Legacy Plans Consolidation)

在长期开发中，`docs/plans/` 目录下往往会积攒大量未执行完毕的 Markdown 计划文档。为了避免“计划吃灰”，我们需要定期进行清理和排序，将其重新纳入总控看板。

### 3.1 定期执行的“收敛指令”

当你发现目录下有很多杂乱的计划时，可以新建一个 AI 会话，发送以下**通用收敛指令**：
...
### 3.2 AI 的执行动作解析
收到上述指令后，AI 将执行以下操作：
1. **自动列出目录**：AI 会调用 `ls` 或 `Glob` 工具读取 `docs/plans/` 下的所有文件。
