...
- [x] **安全授权机制 (Security)**：
    - **默认拒绝策略**：默认 Agent 不再泄露任何未授权工具。
    - **白名单过滤**：严格限制 Agent 只能使用 `enabled_tools` 中配置的工具。
    - **智能指令注入**：根据对话内容智能注入 UML/思维过程指令，避免对普通对话的干扰。
...
- [ ] **AI 自动化增强**：Deep Thinking 模式（进行中）；会话标题 AI 总结与摘要生成能力（后端已落地，待前端配置开关接入）。
...
- [ ] **AI 自动化增强**：Deep Thinking 深度思考模式（进行中）；会话标题 AI 总结与摘要生成（后端完成，管理端待补齐）。
